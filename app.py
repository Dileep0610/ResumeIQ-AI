...
t_resume_load = time.perf_counter() - t0

# Profile Job Description Loading
t0 = time.perf_counter()
jd_text = read_uploaded_file(jd_file) if jd_file else jd_text_input
t_jd_load = time.perf_counter() - t0
...
if st.button("🚀 Build Career Coach RAG Index", type="primary"):
    if not resume_text.strip() or not jd_text.strip():
        ...
        with st.spinner("Running RAG stages: loading → chunking → embeddings → vector database..."):
            vectorstore, num_chunks, timings = get_cached_vectorstore(
                resume_text, 
                jd_text, 
                chunk_size, 
                chunk_overlap
            ...
            print(f"Resume Loading: {t_resume_load:.2f} sec")
            print(f"Job Description Loading: {t_jd_load:.2f} sec")
